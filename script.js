...
const CONFIG = {
    EDAD_MIN: 10,
    EDAD_MAX: 18,
    SCRIPT_URL: 'https://script.google.com/macros/s/AKfycbwsQfB2Ke7l6_xT4vvSdQuPaAXybk16vdns6vyYgSMoz3Yh105--j-QnNxTsXGAh48H/exec'
};
...
const Validator = {
    ...
    validarCampo: (input) => {
        ...
        const esRequerido = input.hasAttribute('data-required') && input.getAttribute('data-required') === 'true';
        ...
        if ((input.id.startsWith('tel') || input.id.startsWith('cel') || input.id === 'contEmerg') && valor) {
             if (!Validator.esTelefonoValido(valor)) {
                Validator.mostrarError(input, 'El teléfono debe tener 11 dígitos (ej. 04141234567).');
                return false;
             }
        }
...
const UI = {
    ...
    stepDescriptions: {
        'step1': "Por favor, completa los datos del estudiante y selecciona las opciones.",
    ...
    async searchByCedula(cedula, searchType) {
        if (Validator.esCedulaValida(cedula)) {
            ...
            try {
                ...
                } else if (result.result === 'not_found' || result.result === 'not_found_debug') {
                    if (result.debug) {
                        ...
                        UI.elements.formStatus.textContent = 'No se encontraron datos. Revisa el cuadro de alerta para más detalles.';
    ...
    showCurrentStep: () => {
        ...
        UI.elements.formDescription.textContent = UI.stepDescriptions[currentStepId];
        
        const progress = ((UI.state.currentStepIndex + 1) / UI.state.visibleSteps.length) * 100;
    ...
    validateCurrentStep: () => {
        ...
        const inputsToValidate = currentStepDiv.querySelectorAll('input, select, textarea');
        ...
        if (fNacEstInput && fNacEstInput.closest('.form-step.active')) {
             ...
             if (isNaN(currentAge) || currentAge < CONFIG.EDAD_MIN || currentAge > CONFIG.EDAD_MAX) {
                 isValid = false;
        ...
        if (!isValid) {
            UI.elements.formStatus.textContent = 'Por favor, corrige los errores marcados.';
    ...
    setupEventListeners: () => {
        ...
        UI.elements.form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (data.result === 'success') {
                    UI.elements.confirmationModal.classList.remove('opacity-0', 'pointer-events-none');
                    UI.elements.confirmationModal.querySelector('.transform').classList.remove('scale-95');
                } else {
        ...
        const cancelCheckboxes = cancelOptions.querySelectorAll('input[type="checkbox"]');
        const cancelButtonText = cancelButton.querySelector('span');

        cancelButton.addEventListener('click', () => cancelOptions.classList.toggle('hidden'));
        ...
        const updateCancelado = () => {
            ...
            cancelButtonText.textContent = labels.length > 0 ? labels.join(', ') : 'Selecciona';
        };
        cancelCheckboxes.forEach(cb => cb.addEventListener('change', updateCancelado));
        
        document.addEventListener('click', (e) => {
            if (!cancelButton.contains(e.target) && !cancelOptions.contains(e.target)) {
